...
static std::tuple<std::string, std::string> ParseShader(const std::string& filepath) {
    ...
    return make_tuple(ss[0].str(), ss[1].str());
}
...
static unsigned int CompileShader(unsigned int type, const std::string& source) {
    ...
    if (result == GL_FALSE) {
        ...
        char* message = (char*)alloca(length * sizeof(char)); //alloca() dynamically allocates on the stack
...
int main(void)
{
    ...
    std::tuple<std::string, std::string> source = ParseShader("./Basic.shader");
    std::string vertex_shader = std::get<0>(source);
    std::string fragment_shader = std::get<1>(source);
